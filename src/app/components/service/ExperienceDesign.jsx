...
const ExperienceDesign = ({ data }) => {
  ...
  return (
    <div
      className={`flex relative  flex-col-reverse gap-4 md:gap-20 h-auto  max-w-[1200px] px-4 mx-auto justify-between p-8 ${data?.isReverse ? " md:flex-row-reverse" : "md:flex-row"
        }`}
    >
      {/* Left Section */}
      <div className="md:w-1/2 w-full">
        ...
        <ul className="space-y-4 w-full">
          {data?.items?.map((item, index) => (
            <div
              key={item.id || index}
              className="md:max-w-[70%] w-full group flex justify-between gap-12 border-b border-silver-100"
            >
              <li
